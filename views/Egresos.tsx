...
export const Egresos: React.FC<Props> = ({ data, onAdd, onDelete }) => (
  <div className="space-y-6">
    <div className="flex justify-between items-center">
      <h1 className="text-2xl font-bold text-gray-900">Gestión de Egresos</h1>
      <button onClick={onAdd} className="bg-red-500 text-white px-4 py-2 rounded-lg flex items-center space-x-2 hover:bg-red-600">
        ...
        <tbody className="divide-y divide-gray-200">
          {data.egresos.map(item => (
            <tr key={item.id}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-red-600">{formatCurrency(item.monto)}</td>
